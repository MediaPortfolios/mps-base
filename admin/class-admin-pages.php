...
class Admin_Pages {
    ...
    public function __construct() {
        ...
        if ( mpb_acf_options() ) {
            ...
            $admin_header = get_field( 'mpb_use_admin_header', 'option' );
        } else {

            // Otherwise look for the WP API setting.
            $admin_header = get_option( 'mpb_use_admin_header' );
        }
    ...
    public function about_plugin() {
        ...
        if ( mpb_acf_options() ) {
            ...
            // Get the field.
            $link_label = get_field( 'mpb_site_plugin_link_label', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_label = sanitize_text_field( get_option( 'mpb_site_plugin_link_label' ) );
        }
        ...
        if ( mpb_acf_options() ) {
            ...
            // Get the field.
            $link_icon  = get_field( 'mpb_site_plugin_link_icon', 'option' );

        // If ACF is not active, get the field from the WordPress options page.
        } else {

            // Get the field.
            $link_icon  = sanitize_text_field( get_option( 'mpb_site_plugin_link_icon' ) );
        }
        ...
        // Otherwise use the scholar's cap icon to imply instruction.
            ...
            $icon = __( 'dashicons-welcome-learn-more', 'mps-text' );
        }
    ...
    public function about_plugin_output() {
        ...
        require MPB_PATH . 'admin/partials/plugin-page-about.php';
    ...
    public function help_about_plugin() {
		...
		$screen->add_help_tab( [
			'id'       => 'help_plugin_info',
			'title'    => __( 'More Information', 'mps-text' ),
			'content'  => null,
		...
		$screen->add_help_tab( [
			...
			'title'    => __( 'Convert Plugin', 'mps-text' ),
			'content'  => null,
	...
	public function help_plugin_info() {
		...
		include_once MPB_PATH . 'admin/partials/help/help-plugin-info.php';
	...
	public function help_convert_plugin() {
		...
		include_once MPB_PATH . 'admin/partials/help/help-plugin-convert.php';
    ...
    public function help_about_page_sidebar() {
        ...
        $html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'mps-text' ) );
        $html .= sprintf(
            '<p>%1s %2s.</p>',
            __( 'This plugin was originally written by', 'mps-text' ),
            'Greg Sweet'
        );
        $html .= sprintf(
            ...
            __( 'for more free downloads.', 'mps-text' )
        );
        $html .= sprintf(
            '<p>%1s</p>',
            __( 'Change this sidebar to give yourself credit for the hard work you did customizing this plugin.', 'mps-text' )
         );
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'mps-text' )
		] );
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once MPB_PATH . 'admin/partials/admin-header.php';
		}
    ...
    public function title_placeholders( $title ) {
        ...
        if ( 'post' == $screen->post_type ) {
            $post_title = esc_html__( 'Post Title', 'mps-text' );

        // Post type: page.
        } elseif ( 'page' == $screen->post_type ) {
            $post_title = esc_html__( 'Page Title', 'mps-text' );
        ...
        // Post type: custom, unidentified.
            ...
            $post_title = esc_html__( 'Enter Title', 'mps-text' );
        }

        // Apply a filter conditional modification.
        $title = apply_filters( 'mpb_post_title_placeholders', $post_title );
    ...
    private function get_column_image( $post_ID ) {
        ...
        $thumb = apply_filters( 'mpb_column_thumbnail_size', $size );
    ...
    public function image_column_head( $defaults ) {
        ...
        $name    = __( 'Featured Image', 'mps-text' );

        // Apply a filter for conditional modification.
        $heading = apply_filters( 'mpb_image_column_head', $name );
